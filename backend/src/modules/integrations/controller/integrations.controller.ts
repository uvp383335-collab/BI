...
export const handleCallback = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (err instanceof AppError && err.code === 'HUBSPOT_ALREADY_CONNECTED_SWITCH') {
    ...
    if (err instanceof AppError && err.code === 'HUBSPOT_ALREADY_CONNECTED_REQUEST_ACCESS') {
      res.redirect(`${FRONTEND_URL}/connect?provider=${provider}&status=error&reason=already_connected_request_access`)
      return
    }
    if (err instanceof AppError && err.code === 'HUBSPOT_ORG_LOCKED_TO_ACCOUNT') {
