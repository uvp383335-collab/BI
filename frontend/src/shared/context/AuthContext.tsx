...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setSession = useCallback(
    (accessToken: string, org: OrganizationSummary) => {
      ...
      // Virtually every query in this app is org-scoped (integrations, sync status,
      // analytics, ...) and none of their query keys include orgId, so a stale cache
      // entry from the previous org would otherwise keep rendering after a switch.
      // Invalidate everything so active queries refetch under the new session.
