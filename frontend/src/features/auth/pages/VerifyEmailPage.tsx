...
export const VerifyEmailPage: React.FC = () => {
  ...
  return (
    <AuthCard title="Email verification">
      ...
      {status === 'success' && (
        <div>
          <p className="text-sm text-green-700 mb-4">Your email has been verified successfully.</p>
      ...
      {status === 'error' && (
        <div>
          <p className="text-sm text-red-600 mb-4">This verification link is invalid or has expired.</p>
          <Link to="/login" className="auth-link text-sm">
            Back to log in
          </Link>
        </div>
