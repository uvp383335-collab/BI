...
export const CrmDashboardPage: React.FC = () => {
  ...
  // Defaults to last year, not docs/server.js's 2018 — the trend cards only ever have
  // ~18 months of synced history (PLSnapshot/PLItemSnapshot's trailing-6-quarters window),
  // so a much older default would render mostly "not computable" gaps and an unreadably
  // dense x-axis by default. The control itself is unrestricted — typing an earlier year
  // still works, it's just not the default.
  const [quickbooksFromYear, setQuickbooksFromYear] = useState(() => String(new Date().getFullYear() - 1))
  ...
  const { data: vc13, isLoading: isVc13Loading } = useMetric('vc-13', quickbooksMetricsEnabled, undefined, quickbooksItemFilter, quickbooksStateFilter || undefined, quickbooksRevenueViewMode)
  const { data: vc13Trend, isLoading: isVc13TrendLoading } = useMetricTrend('vc-13', quickbooksMetricsEnabled, quickbooksItemFilter, quickbooksFromYearParam, quickbooksStateFilter || undefined, quickbooksRevenueViewMode)
      ...
      <div className="mx-auto w-full max-w-[1600px] space-y-8">
        ...
        {hasNeverSynced ? (
          ...
          <>
            ...
            {/* Level 1 — Executive summary: the headline financial KPIs, always visible, never collapsed. */}
            {isQuickbooksVisible && (
              <section className="space-y-4">
                <div>
                  <h2 className="text-xl font-semibold text-ink sm:text-2xl">Executive summary</h2>
                  <p className="text-sm text-ink-2">The headline financial health numbers, at a glance.</p>
                </div>
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
                  <KpiTile label="Revenue Growth (YoY)" metric={vc13} isLoading={isVc13Loading} />
                  <KpiTile label="Gross Revenue Retention" metric={vc01} isLoading={isVc01Loading} />
                  <KpiTile label="Net Revenue Retention" metric={vc02} isLoading={isVc02Loading} />
                  <KpiTile label="Recurring Revenue %" metric={vc12} isLoading={isVc12Loading} />
                </div>
              </section>
            )}

            {/* Connected data — sync/volume health, intentionally smaller and secondary to the business KPIs above. */}
            <section className="space-y-4">
              <div>
                <h2 className="text-lg font-semibold text-ink">Connected data</h2>
                <p className="text-sm text-ink-2">What's synced from your connected sources right now.</p>
              </div>
            ...
            {/* Level 3 — Sales & Pipeline analysis. */}
            {(isHubspotVisible || isSalesforceVisible) && (
              <CollapsibleSection
                title="Sales & Pipeline"
                description="Lead and deal stage progression, and lead-to-deal conversion."
                headerContent={
                  bothCrmsVisible ? (
            ...
            {/* Revenue & Margins — retention/growth/cost-structure, all filterable by product and starting year. */}
            {isQuickbooksVisible && (
              <CollapsibleSection
                title="Revenue & Margins"
                description="Retention, growth, and cost structure. Filter the product-aware metrics by product and starting year."
                headerContent={
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {quickbooksDepartments && quickbooksDepartments.length > 0 && (
                      <select className="form-input-dark w-auto" value={quickbooksLocationFilter} onChange={(e) => setQuickbooksLocationFilter(e.target.value)} aria-label="Location">
              ...
              >
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                  ...
                  <MetricCard
                    ...
                    controls={
                      <>
                        {quickbooksCustomerStates && quickbooksCustomerStates.length > 0 && (
                          <select className="form-input-dark w-auto" value={quickbooksStateFilter} onChange={(e) => setQuickbooksStateFilter(e.target.value)} aria-label="Billing state">
                    ...
                    trend={<MetricTrendChart embedded title="Revenue Growth %" description="" color="var(--color-success)" points={vc13Trend?.points ?? []} isLoading={isVc13TrendLoading} viewMode={quickbooksRevenueViewMode} metricId="vc-13" />}
            ...
            {isQuickbooksVisible && (
              <CollapsibleSection title="Unit Economics & Customers" description="Acquisition efficiency, lifetime value, and revenue concentration.">
            ...
            {/* Cash. */}
            {isQuickbooksVisible && (
              <CollapsibleSection title="Cash" description="Runway, conversion, and how long cash stays tied up in the operating cycle.">
            ...
            {/* Marketing & Pipeline, per connected CRM — stage names/pipeline data can't be blended across CRMs. */}
            {isHubspotVisible && (
              <CollapsibleSection title={`Marketing & Pipeline${suffix('HubSpot')}`} description="Qualified pipeline, funnel conversion, and marketing-sourced revenue.">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
                  <MetricCard id="CM-04" title="Pipeline Coverage" description="Qualified pipeline closing next quarter vs. target" icon={<Target className="h-5 w-5" />} metric={cm04Hubspot} isLoading={isCm04HubspotLoading} />
                  <MetricCard id="CM-06" title="Funnel Conversion (MQL -> SQL)" description="Cohort-based stage-to-stage advancement" icon={<Users className="h-5 w-5" />} metric={cm06Hubspot} isLoading={isCm06HubspotLoading} />
                  <MetricCard id="CM-08" title="MQL Volume" description="Marketing-qualified leads generated this period" icon={<LineChart className="h-5 w-5" />} metric={cm08Hubspot} isLoading={isCm08HubspotLoading} />
            ...
            {isSalesforceVisible && (
              <CollapsibleSection title={`Marketing & Pipeline${suffix('Salesforce')}`} description="Qualified pipeline, funnel conversion, and competitive win rate.">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
